...
import com.junkj.common.collect.SetUtils;
...
public class PropertiesUtils {
	...
	private static final class PropertiesLoaderHolder {
		...
		public static void releadInstance() {
			...
			Resource[] resources = ResourceUtils.getResources("classpath*:/config/jun-*.*");
			for (Resource resource : resources) {
				configSet.add("classpath:config/" + resource.getFilename());
			}
			// configSet.add("classpath:config/jun.yml");
			...
			if (StrUtils.isNotBlank(customConfigs)) {
				for (String customConfig : StrUtils.split(customConfigs, ",")) {
			...
			for (String location : configFiles) {
				...
				if (StrUtils.isNotBlank(profiles)) {
					if (location.endsWith(".properties")) {
						configSet.add(StrUtils.substringBeforeLast(location, ".properties") + "-" + profiles
								+ ".properties");
					} else if (location.endsWith(".yml")) {
						configSet.add(StrUtils.substringBeforeLast(location, ".yml") + "-" + profiles + ".yml");
					}
				}
			}
			configFiles = configSet.toArray(new String[configSet.size()]);
			logger.debug("Loading jun config: {}", (Object) configFiles);
	...
	public String getProperty(String key) {
		...
		if (value != null) {
			...
			while (m.find()) {
				...
				value = StrUtils.replace(value, g, getProperty(childKey));
			}
			return value;
		} else {
	...
	private static Logger initLogger() {
		...
		if (StrUtils.isBlank(System.getProperty("logPath"))) {
