...
public class StaffApiAction extends BaseAction {
	 ...
	 * 改变我的消息状态
	 */
	@RequestMapping(value = "tplInfo/{comId}/{id}", method = RequestMethod.GET)
	public String tplInfo(@PathVariable String comId, @PathVariable String id) {
		staffApiBiz.updateMyWorkMsg(id);
		WorkMsg workMsg = workMsgBiz.get(id);
		return sendRedirect(
				"http://" + ComUtils.getCurrentComId() + "." + Global.mobileUri() + "/mstaff#" + workMsg.getUrl());
	}
