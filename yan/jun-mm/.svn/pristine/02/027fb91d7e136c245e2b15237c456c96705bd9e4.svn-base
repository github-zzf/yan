...
public class ComUtils {
	...
	private static ComConfigBiz comConfigBiz = SpringUtils.getBean(ComConfigBiz.class);
	 ...
	 * 获取企业配置
	 */
	public static String getComConfig(String comId, String key) {
		String val = getCache(comId, CACHE_COM_TPL_ + key);
		if (val == null) {
			val = comConfigBiz.getByKey(comId, key);
			if (val != null) {
				putCache(comId, CACHE_COM_TPL_ + key, val);
			}
		}
		return val;
	}
	...
	public static String getComUrl(String path) {
		return getComUrl(getCurrentComId(), path);
	}

	public static String getComUrl(String comId, String path) {
		if (Global.isDev()) {
			return Global.devUri() + path;
		} else {
			return "http://" + comId + "." + Global.mobileUri() + path;
		}
	}
