package com.junkj.common.codec;
...
//	}
	...
	public static String xssFilter(String text) {
		String oriValue = StrUtils.trim(text);
		if (text != null){
			String value = oriValue;
			for (Pattern pattern : xssPatterns) {
				Matcher matcher = pattern.matcher(value);
				if (matcher.find()) {
					value = matcher.replaceAll(StrUtils.EMPTY);
				}
			}
			// 如果开始不是HTML，XML，JOSN格式，则再进行HTML的 "、<、> 转码。
			if (!StrUtils.startsWithIgnoreCase(value, "<!--HTML-->") 	// HTML
					&& !StrUtils.startsWithIgnoreCase(value, "<?xml ") 	// XML
					&& !StrUtils.contains(value, "id=\"FormHtml\"") 		// JFlow
					&& !(StrUtils.startsWith(value, "{") && StrUtils.endsWith(value, "}")) // JSON Object
					&& !(StrUtils.startsWith(value, "[") && StrUtils.endsWith(value, "]")) // JSON Array
				){
