package com.junkj.common.IdGen;
...
public class IdGenerate {
	...
	public static String nextCode(String code){
		if (code != null){
			...
			long num = NumUtils.isCreatable(numStr) ? Long.valueOf(numStr) : 0;
//			System.out.println("处理前："+str);
			str = prefix + StrUtils.leftPad(String.valueOf(num + 1), numStr.length(), "0");
//			System.out.println("处理后："+str);
	...
	public static String getRandomNum(int length) { // length表示生成字符串的长度
		...
		for (int i = 0; i < length; i++) {
			int number = random.nextInt(numChars.length);
			sb.append(numChars[number]);
		}
		return sb.toString();
	}
