...
var app = {
	...
	bind : function() {
		...
		$('#user_xtable').on("click", ".btn_status", function(){
			...
			$.xalert({
				...
				okBack:function(){
					$.xpost({
						url:adminPath + '/sysUser/updateStatus',
		...
		$('#user_xtable').on("click", ".btn_resetPass", function(){
			...
			$.xalert({
				...
				okBack:function(){
					$.xpost({
						url : adminPath + '/sysUser/resetPass',
						data:{id:id},
						success:function(e){
							if (e.code==1) {
								$.ok("设置成功")
	...
	load:function(){
		app.table = $('#user_xtable').xtable({
			...
			deleteUrl:adminPath+'/sysUser/delete',
			paging:'.user_paging',
			format:function(row){
			...
			update:function(row){
				...
				$("#f_comName").addClass("readonly").attr("readonly","readonly");
				$("#f_username").addClass("readonly").attr("readonly","readonly");
				$("#f_comId").unbind("click");
