...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
	...
	private static String[] parsePatterns = { "yyyy-MM-dd", "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd HH:mm", "yyyy-MM-dd HH",
			...
			"yyyy年MM月dd日 HH时mm分ss秒", "yyyy年MM月dd日 HH时mm分", "yyyy年MM月dd日 HH时", "yyyy年MM月", "yyyy", "HH:mm:ss", "HH:mm" };
	 ...
	 *            数量，前为负数，后为正数
	 * @param type
	 *            类型，可参考Calendar的常量(如：Calendar.HOUR、Calendar.MINUTE、Calendar.
	 *            SECOND)
	 * @return
	 */
	...
	/**
	 * 日期比较: d1 大于 d2 返回 true
	 */
	public static boolean after(String s1, String s2, String format) {
		try {
			Date d1 = parseDate(s1, format);
			Date d2 = parseDate(s2, format);
			return after(d1, d2);
		} catch (ParseException e) {
			return false;
		}
	...
	/**
	 * 日期比较: d1 小于 d2 返回 true
	 */
	public static boolean before(String s1, String s2, String format) {
		try {
			Date d1 = parseDate(s1, format);
			Date d2 = parseDate(s2, format);
			return before(d1, d2);
