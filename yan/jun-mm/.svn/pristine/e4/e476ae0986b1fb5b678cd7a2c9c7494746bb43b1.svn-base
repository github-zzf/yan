...
@Table(name = "com_activity_enroll", alias = "a", columns = {
		...
		@JoinTable(type = JoinTable.Type.LEFT_JOIN, entity = Activity.class, alias = "ac", on = "ac.id = a.activity_id ", columns = {
			@Column(name = "name", attrName = "activityName", queryType = QueryType.LIKE),
			@Column(name = "start_time", attrName = "activityStartTime", queryType = QueryType.EQ),
			@Column(name = "end_time", attrName = "activityEndTime", queryType = QueryType.EQ) 
		}),
		@JoinTable(type = JoinTable.Type.LEFT_JOIN, entity = SysBeanData.class, alias = "apy", on = "apy.bean_key='activity_py_state' and apy.bean_val = a.pay_state", columns = {
			@Column(name = "bean_txt", attrName = "payStateTxt", queryType = QueryType.LIKE)
		})
	}, orderBy = "a.id desc")
public class ActivityEnroll extends DataEntity<ActivityEnroll> {
	...
	private String enrollStatus; // 状态（0正常、1取消、4等位）
	...
	private String payStateTxt; // 支付状态
