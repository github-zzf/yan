(function($) {
	...
	$(function() {
		...
		// 参数名
		$type = $wrap.attr('biztype'),

		// 添加数据
		$add = $('<input type="hidden" id="' + $type + '" name="' + $type + '"/>')
				.appendTo($wrap),

		// 删除数据
		$del = $(
				'<input type="hidden" id="' + $type + '_del" name="' + $type + '_del"/>').appendTo($wrap),
	    ...
	    var callback = function(mutationsList) {
	        mutationsList.forEach(function(item,index){
	            if (item.type == 'attributes') {
	                var $id = $wrap.attr(item.attributeName);
	                
	                
	                $.ajax(adminPath+'/file/fileList', {
						method : 'POST',
						data : {bizId:$id, bizType:$type},
						dataType : 'json'
						...
						if (response.code==1) {
							...
							$.each(response.data,function(i, item){
			                    ...
			                    obj.size = item.fileEntity.fileSize;
			                    obj.lastModifiedDate = item.createDate;
			                    obj.id = item.id;
			                    obj.ext = item.fileEntity.fileExtension;
		...
		function addFile(file) {
			var $li = $('<li id="' + file.id + '" imageId="' + file.source.id + '">' + '<p class="title">'
			...
			if (file.source.fileUrl) {
				var img = $('<img src="' + file.source.fileUrl + '">');
