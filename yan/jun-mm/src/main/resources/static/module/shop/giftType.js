...
var app = {
	...
	bind : function() {
		...
		$('#btn_export').xbtn(function() {
			...
			var url = adminPath + '/giftType/excelDownload?='+data;
			linkExcel.open("shop_gift_type", url);
		})
		// 添加
		$('#btn_add').click(function(){
			app.form.clear();
		...
		// 添加子集
		$('#giftType_xtable').on("click",".btn_addSon",function(){
	...
	load:function(){
		...
		app.table = $('#giftType_xtable').xtable({
			url:adminPath + '/giftType/findList',
			deleteUrl:adminPath + '/giftType/delete',
			tree: true,
			format:function(row){
			...
			success:function(){
				$('#giftType_xtable').find(".tree_click").trigger("click");
			}
		});
	}
};