...
var app = {
	...
	bind : function() {
		...
		$('#btn_export').xbtn(function() {
			...
			var url = adminPath + '/level/excelDownload?='+data;
			linkExcel.open("member_level", url);
		})
	},
	load:function(){
		app.table = $('#level_xtable').xtable({
			url:adminPath + '/level/listPage',
			deleteUrl:adminPath + '/level/delete',
			paging:'.level_paging',
			format:function(row){
