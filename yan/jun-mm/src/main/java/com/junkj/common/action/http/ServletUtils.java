package com.junkj.common.action.http;
...
public class ServletUtils {
	...
	public static boolean isAjaxRequest(HttpServletRequest request) {
		...
		if (StrUtils.endsWithIgnoreCase(uri, ".json") || StrUtils.endsWithIgnoreCase(uri, ".xml")) {
			return true;
		}
	...
	public static boolean isStaticFile(String uri) {
		if (staticFiles == null) {
			...
			try {
				staticFiles = StrUtils.split(pl.getProperty("web.staticFile"), ",");
				staticFileExcludeUri = StrUtils.split(pl.getProperty("web.staticFileExcludeUri"), ",");
		...
		if (staticFileExcludeUri != null) {
			for (String s : staticFileExcludeUri) {
				if (StrUtils.contains(uri, s)) {
					return false;
				}
			}
		}
		if (StrUtils.endsWithAny(uri, staticFiles)) {
			return true;
		}
		return false;
	}
...
//			response.reset(); // 注释掉，否则以前设置的Header会被清理掉，如ajax登录设置记住我的Cookie信息
			if (type == null) {
				if ((StrUtils.startsWith(string, "{") && StrUtils.endsWith(string, "}"))
						|| (StrUtils.startsWith(string, "[") && StrUtils.endsWith(string, "]"))) {
					type = "application/json";
				} else if (StrUtils.startsWith(string, "<") && StrUtils.endsWith(string, ">")) {
	...
	public static Map<String, Object> getExtParams(ServletRequest request) {
		...
		String params = StrUtils.trim(request.getParameter(DEFAULT_PARAMS_PARAM));
		if (StrUtils.isNotBlank(params) && StrUtils.startsWith(params, "{")) {
			paramMap = JsonUtils.toObject(params, Map.class);
		} else {
	...
	/**
	 * 根据浏览器If-Modified-Since Header, 计算文件是否已被修改. 如果无修改, checkIfModify返回false ,设置304
	 * not modify status.
	 * 
	...
	public static Integer getParameterToInt(String name) {
		return NumUtils.toInt(getRequest().getParameter(name));
	}

	public static Integer getParameterToInt(String name, Integer defaultValue) {
		return NumUtils.toInt(getRequest().getParameter(name), defaultValue);
	}
