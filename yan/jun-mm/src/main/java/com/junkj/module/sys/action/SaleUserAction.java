 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/saleUser")
public class SaleUserAction extends BaseAction {
	...
	public JsonVo updateStatus(SysUser sysUser) {
		...
		if (!SysUser.USER_TYPE_SALE.equals(sysUser.getUserType())) {
			return sendError("不允许操作此用户！");
		}
	...
	public Object delete(SysUser sysUser) {
		...
		if (!SysUser.USER_TYPE_SALE.equals(user.getUserType())) {
			return sendError("不允许操作此用户！");
		}
	...
	public JsonVo save(SysUser sysUser) {
		...
		if (sysUser.getIsNewT()) {
			sysUser.setUserType(SysUser.USER_TYPE_SALE);
	...
	public JsonVo resetPass(SysUser sysUser) {
		...
		if (!SysUser.USER_TYPE_SALE.equals(u.getUserType())) {
			return sendError("不允许操作此用户！");
		}
