		...
		if (list != null) {
			list.forEach(workMsg -> {
				...
				if (wxUser != null && StrUtils.notBlank(wxUser.getOpenid())) {
					...
					data.add(text);
					data.add("今天又是愉快的一天！");
					String url = ComUtils.getComUrl(comId, "/staff/tplInfo/" + comId + "/" + workMsg.getId());
					TplMsgUtils.sendMsg(comId, wxUser.getOpenid(), url, TplMsgUtils.TPL_10, data);
				}
			});
		}
	}
		...
		if (list != null) {
			list.forEach(enroll -> {
				...
				if (StrUtils.notBlank(enroll.getOpenId())) {
					String comId = enroll.getComId();
					List<String> data = ListUtils.newArrayList();
					data.add("您还未对以下课程进行评价");
					data.add(enroll.getCourseName());
					data.add(
							DateUtils.format(enroll.getCourseDate()) + DateUtils.format(enroll.getEndTime(), " HH:mm"));
					data.add("服务老师：" + enroll.getTeacherName());
					String url = ComUtils.getComUrl(comId, "/mobile#/pages/nav/service/service?type=my");
					TplMsgUtils.sendMsg(comId, enroll.getOpenId(), url, TplMsgUtils.TPL_7, data);
				}
			});
		}
	}
